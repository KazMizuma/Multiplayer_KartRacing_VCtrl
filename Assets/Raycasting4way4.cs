...
public class Raycasting4way4 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        Ray forwardRay = new Ray(gameObject.transform.position, gameObject.transform.forward * 4f); // 6/28 Trfc Ctrl, raycasting to the left
        Debug.DrawRay(gameObject.transform.position, gameObject.transform.forward * 4f, Color.green);

        if (Physics.Raycast(forwardRay, out hitTrfc, 4f))
        {
