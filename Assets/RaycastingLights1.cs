...
public class RaycastingLights1 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        Ray rightRay = new Ray(gameObject.transform.position, gameObject.transform.forward * 20f); // 6/28 Trfc Ctrl, raycasting to the left
        Debug.DrawRay(gameObject.transform.position, gameObject.transform.forward * 20f, Color.green);

        if (Physics.Raycast(rightRay, out hitTrfc, 20f))
        {
